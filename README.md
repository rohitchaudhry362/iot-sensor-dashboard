...
- Can you build a user interface that works and demonstrates good judgment?
...
- Both frontend and backend are written in TypeScript and already have a `tsconfig.json`.
- The final deliverable must be fully runnable in Docker: a single `docker compose up` on a fresh clone should start the frontend, backend, Mosquitto, and your database (Task 1.0) together.
...
- The activity.json contains raw time series data in 15-minute buckets, where `activity` is the number of minutes with motion in that bucket (0 to 15).
- The sensors.json contains events from two devices: a temperature/humidity sensor in a bathroom (`SensorValueChanged`, values in `payload`) and a vibration sensor on the front door (`SensorDetected`, empty payload).
- Pick a database solution of your choice (in docker) and store the data to fetch in the application.

#### Task 1.1: Real-Time Communication Layer

**The MQTT broker is Mosquitto** (`eclipse-mosquitto` in your compose file). Your simulated sensors publish to it and your backend subscribes to it; how the data then reaches the browser is up to you.
...
**Implement:**
- Server-side: Get MQTT data from Mosquitto to the browser.
...
- [ ] App, Mosquitto and database run without errors on a fresh clone with `docker compose up`

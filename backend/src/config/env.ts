...
const schema = Joi.object<Env>({
  NODE_ENV: Joi.string().valid('development', 'production', 'test').default('development').example('production'),
  PORT: Joi.number().port().default(4000).example(4000),
  LOG_LEVEL: Joi.string().valid('error', 'warn', 'info', 'http', 'debug').default('info').example('debug'),
  CORS_ORIGIN: Joi.string().uri().default('http://localhost:3000').example('https://app.example.com'),

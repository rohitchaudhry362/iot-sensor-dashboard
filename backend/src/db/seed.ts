...
const seedSensorEvents = async (
  ...
  await tx.location.createMany({
    data: [...locations.values()].map((e) => ({ networkId: networkIds.get(e.networkId) as number, name: e.locationName })),
    skipDuplicates: true,
  })
  const locationIdByKey = new Map(
    (await tx.location.findMany()).map((l) => [`${l.networkId}|${l.name}`, l.id]),
  )
...
const seedActivities = async (
  ...
  await inBatches(buckets, (batch) =>
    tx.activity.createMany({
      data: batch.map((b) => ({ networkId: networkIds.get(b.networkId) as number, time: b.time, activity: b.activity })),
      skipDuplicates: true,
    }),
  )

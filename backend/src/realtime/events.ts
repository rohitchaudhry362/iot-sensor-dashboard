import type { NetworkStatus } from '../services/networkPresence';

// Timestamps are ISO strings because JSON has no date type; the browser turns them back into dates.
// Every event carries its network id so it is self-describing
...
export type ServerToClientEvents = {
  ...
  'network:status': (event: NetworkStatusEvent) => void;
};
